...
//#footnote[#link("https://courses.fit.cvut.cz/NI-DSV/tutorials/files/topologies.txt")[#text(fill: blue,underline("https://courses.fit.cvut.cz/NI-DSV/tutorials/files/topologies.txt"))]
  ...
  show raw.where(block: true): it => {
    block(fill: rgb(229, 228, 226, 80),inset: 8pt, radius: 4pt,  
    grid(
      columns: (auto,90%),
      align: left+top,
      column-gutter: 0.5em,
      stroke: (x,y) => if x==0 {( right: (paint:gray, dash: "dashed") )},
      inset: 0.3em,
      ..it.lines.map((line) => (str(line.number), text(line.body, size: 0.9em, font: "Noto Sans Mono"))).flatten()
      )
    )
  }
  ...
  place(
    ...
    clearance: 30pt,
    {
      ...
      v(3.35mm, weak: true)
      align(center, par(leading: 0.5em, text(size: 10pt, subtitle)))
      v(8.35mm, weak: true)
  ...
  if used-shorts != none [
    #set text(8pt)
    #set block(inset: (left: 10%))
    #heading(numbering: none, text(10pt)[List of Abbreviations])
      #grid(
        columns: (auto, auto),
        column-gutter: 20pt,
        row-gutter: 8pt,
